...
export function ToolboxTalkForm({
  ...
  const fetchUserFullName = async () => {
    try {
      ...
      if (user?.user_metadata?.full_name) {
        setFormData((prev) => ({
          ...prev,
          presenter: user.user_metadata.full_name,
        }));
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user)
        throw new Error('You must be logged in to create a toolbox talk.');
      ...
      setError(
        err instanceof Error
          ? err.message
          : 'An unexpected error occurred while saving the toolbox talk.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const selectedPDFFile = pdfFiles.find(
    (file) => file.name === formData.selectedPDF
  );
            ...
            <div className="max-h-[300px] overflow-y-auto pr-2">
              <div className="space-y-4">
                {formData.attendees.map((attendee) => (
                  <div
                    key={attendee.id}
                    className="flex flex-col sm:flex-row sm:items-start sm:space-x-4 space-y-2 sm:space-y-0 bg-gray-50 p-4 rounded-lg"
                  >
                    <div className="flex-1">
                      <input
                        ...
                        value={attendee.name}
                        onChange={(e) =>
                          handleAttendeeNameChange(attendee.id, e.target.value)
                        }
