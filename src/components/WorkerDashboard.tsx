...
import { LogOut, HardHat, AlertTriangle, FileText, User, Moon, Sun, Shield, Menu, X } from 'lucide-react';
...
export function WorkerDashboard() {
  ...
  const [needsHealthQuestionnaire, setNeedsHealthQuestionnaire] = useState<boolean>(true);
  ...
  // Add a new effect to check health questionnaire status when user data changes
  useEffect(() => {
    if (user?.last_health_questionnaire) {
      const lastCompletionDate = new Date(user.last_health_questionnaire);
      const sixMonthsAgo = new Date();
      sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);
      setNeedsHealthQuestionnaire(lastCompletionDate < sixMonthsAgo);
    } else {
      setNeedsHealthQuestionnaire(true);
    }
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      // Fetch worker details from workers table using email
      ...
      // Update the user state with the worker data
      setUser(workerData);
  ...
  const refreshUserData = async () => {
    ...
    try {
      ...
      console.log('Fetching updated worker data for user email:', authUser.email);
      ...
      if (workerError) {
        ...
        console.log('Last health questionnaire date:', workerData?.last_health_questionnaire);
      }
      ...
      console.log('Health questionnaire date:', updatedUserData.last_health_questionnaire);
  ...
  const isHealthQuestionnaireNeeded = (): boolean => {
    ...
    console.log('Checking if health questionnaire is needed. Last date:', lastHealthQuestionnaire);
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm">
          ...
          <div className="flex justify-between h-16">
            <div className="flex">
              <div className="flex-shrink-0 flex items-center">
                <img src="/opg-logo.svg" alt="Logo" className="h-8 w-8" />
                <a href="/" className="ml-2 text-xl font-bold text-gray-900 dark:text-white hover:text-amber-500 dark:hover:text-amber-400">
                  Worker Portal
                </a>
              </div>
            </div>
        ...
        <div className={`md:hidden ${isMobileMenuOpen ? 'block' : 'hidden'}`}>
          <div className="px-2 pt-2 pb-3 space-y-1">
            <button
              onClick={toggleDarkMode}
              className="w-full flex items-center px-3 py-2 text-base font-medium rounded-md text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600"
            >
              {isDarkMode ? (
            ...
            <button
              onClick={handleSignOut}
              className="w-full flex items-center px-3 py-2 text-base font-medium rounded-md text-white bg-[#DC2626] hover:bg-red-700"
            >
              <LogOut className="h-4 w-4 mr-2" />
              Sign out
            </button>
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
            ...
            {user?.last_health_questionnaire ? (
              <p className="text-amber-600 dark:text-amber-400 mt-1">
                Health Questionnaire completed on {new Date(user.last_health_questionnaire).toLocaleDateString('en-GB')}. 
                Next review will be 6 months time {new Date(new Date(user.last_health_questionnaire).setMonth(new Date(user.last_health_questionnaire).getMonth() + 6)).toLocaleDateString('en-GB')}
              </p>
            ) : (
              <p className="text-red-600 dark:text-red-400 mt-1">
                Health Questionnaire has not been completed. You will need to do this every 6 months before you scan a site QR code.
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div 
              ...
              onClick={hasValidHealthQuestionnaire() ? handleScanQRCode : undefined}
            >
            ...
            <div 
              ...
              onClick={isHealthQuestionnaireNeeded() ? handleHealthQuestionnaire : undefined}
            >
      ...
      <footer className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-4 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-gray-500 dark:text-gray-400 text-sm">
            &copy; {new Date().getFullYear()} On Point Groundworks. All rights reserved.
          </p>
        </div>
      </footer>
