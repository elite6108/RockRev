...
// JSX runtime and SVG types are now defined in vite-env.d.ts
...
export function QRScannerModal({
  ...
  const startCamera = async () => {
    ...
    try {
      // First attempt: Try with simple constraints for desktop compatibility
      // Skip this on mobile devices as we want to ensure we get the back camera
      const isMobileDevice = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
      
      if (!isMobileDevice) {
        try {
          console.log('Attempt 1: Using simple constraints for desktop compatibility');
        ...
        console.log('Mobile device detected, skipping simple constraints to ensure back camera usage');
      }
      
      // Second attempt: Try to enumerate devices and select appropriate camera
      ...
      if (isMobileDevice) {
        // On mobile devices, always use the back camera
        // This is more reliable than checking for multiple cameras
        console.log('Using back camera for mobile device');
        constraints = {
          video: {
        ...
        // For desktop, try to find a back-facing camera if available
        // Some laptops/desktops might have multiple cameras
        console.log('Selecting best camera for desktop');
        
        // Try to find a camera with 'back' in the label (some devices label cameras)
        ...
        if (backCamera) {
          console.log('Found camera labeled as back/rear camera');
          constraints = {
            video: {
